...
//Fonction de Hill
// utilisée dans les formules du système. 
...
W0 = K4; // supposons à K4

[Wst, feq] = fsolve(W0, list(Wintersection, par));
m_st = (a1/gM) * ( Wst^n /(Wst^n + K1^n));
Pc_st = (a2 / (a3 + gP)) * m_st;
Pn_st = (a3 / (gP + gamma * Wst)) * Pc_st;

disp(Wst, feq) // on obtient Wst ~= 0.27 et feq environ 0 donc le modèle est valide. 
// Définition de W0 ou "initial guess" Et définition de tous les points d'équilibre 

scf(3);
plot(Wst, (1/gamma) * (a4 * ((Wst^(n-1) / (K4^n + Wst^n)) - gP)), "k*" ,"linewidth", 3); // affiche le point d'intersection
xlabel("W");
xgrid(1);
...
A = [];
// dW/dt
A(1,1) = (a4 * n * Wst^(n-1) / (K4^n + Wst^n)^2) -gP -gamma * Pn_st; // Dérivée partielle des équations de base par rapport à W
A(1,2) = 0; // Par rapport à m
A(1,3) = 0; // Par rapport à Pc
A(1,4) = -gamma * Wst; // Par rapport à Pn
// 2e ligne matrice / 2e équation dm/dt
A(2,1) = (a1 * n * K1^n * Wst^(n-1)) / (K1^n + Wst^n)^2;
A(2,2) = -gM;
A(2,3) = 0;
A(2,4) = 0;
// dPc/dt
...
// Calcul matrice jacobienne à Wst = Pc_st = Pn_st = m_st = 0
A0 = [];
...
function C = fonction_cout(p)
    ...
    // Calcul des amplitudes maximales et minimales normalisée par rapport à la valeur max
    Xmax = max(xsol(1,LX:L)) / max(xsol(1,LX:L));
    Xmin = min(xsol(1,LX:L)) / max(xsol(1,LX:L));
    ...
    // total frequence :
    Ftot=xsol(2,LX:L);
    Xminfrq=min(Ftot(LX:L))/max(Ftot(LX:L));
    Xminfrqobs=1/5
    
    // Calcul de la fonction de coût
    C = (periodeS - observed_period)^2 + (Xmax - observed_Xmax)^2 + (Xmin - observed_Xmin)^2+(Xminfrq-Xminfrqobs)^2;
...
initial_guess_params = [8, 7, 0.5, 40, 0.15, 0.4, 0.01, 50, 0.7, 2];
...
function [liste_periodes, liste_a3] = modif_a3(par)
    liste_periodes = [];
    liste_a3 = [];
    while par(3) < 10
        ...
        tvec = t0:dt:150;
        par(3) = par(3) * 1.2;
        xsol = ode("stiff", X0, t0, tvec, list(clock_model_rhs, par)); // On résout le système avec les nouveaux paramètres
        liste_periodes = [liste_periodes, calculperiode(xsol(1,:), tvec)]; // On calcule la période d'oscillation du nouveau système
...
[periodes, a3s] = modif_a3(initial_guess_params);

figure (13);
plot(a3s, periodes, "ro-", 'LineWidth', 2); 
xtitle('Evolution de la période en fonction des a3', 'a3', 'Période');
//f3.background = color("white");
